...
dades<-dades%>%mutate(HbA1c_7=ifelse(HBA1c.valor>=7.5,"Si","No"))
#
#
...
```{r JOPSEP: Baseline characteristics of the study variables between diabetics with and withou TB,include=F,eval = F}
...
dades_DM<-dades %>% filter(grup=="Diabetis")

#table(dades_DM$EV.TBC_cat)
...
descrTable(formu,data=dades_DM,show.p.overall = T,show.n = T,method = 1,Q1=0,Q3=1) %>%
  export2md(caption = "Características basales de las variables de estudio entre diabéticos con y sin TBC [Medias,Sd]")

descrTable(formu,data=dades_DM,show.p.overall = T,show.n = T,method = 2,Q1=0,Q3=1) %>%
  export2md(caption = "Características basales de las variables de estudio entre diabéticos con y sin TBC [Medianas,Min,Max]")


dades_DM2<-dades %>% filter(grup=="Diabetis" & EV.TBC_cat=="Si")
dades_DM3<-dades %>% filter(grup=="Diabetis" & EV.TBC_cat=="No")



ggplot(data = dades_DM2, aes(HBA1c.valor))+
...
ggplot(data = dades_DM3, aes(HBA1c.valor))+
   geom_histogram(bins = 10,aes(y =1*( ..count../sum(..count..)))) +
    scale_y_continuous(name = "%", labels=scales::percent) +
...
#dades_DM2<-dades %>% filter(EV.TBC_cat=="Si")

# T BASAL2 -------------
#formu2<-formula_compare(x="basalesJosep",y="HbA1c_7",taulavariables = conductor_variables)

#descrTable(formu,data=dades_DM2,show.p.overall = T,show.n = T,method = 1,Q1=0,Q3=1) %>%
#  export2md(caption = "Baseline characteristics of the study variables between diabetics with and withou TB")
...
```{r exploratori, message=FALSE, warning=FALSE, include=T, echo=FALSE,size="huge",eval = T}
# T BASAL -------------
...
```{r resultats1, include=T, warning=FALSE,echo=FALSE, message=F,eval = T}
formu<-formula_compare(x="basales",y="grup",taulavariables = conductor_variables)
...
kkk<-dt_agencia %>%select(CIP,TB__PULMONAR,TB_PLEURAL,TB_PLEUROPULM,TB_LIMFATICA,TB_OSTEOART,TB_MILIAR,TB_GENITOURIN,TB_MENINGEA,TB_LARINGEA,TB_ALTRESLOC,PPD,RADIOLOGIA,PPD)%>% unique()
kkk<-kkk%>% filter( CIP!="GARR144010100")
...
descrTable(formu,data=dades %>% filter(grup=="Diabetis"),show.p.overall = T,show.n = T,method = 1,Q1=0,Q3=1) %>%
  export2md(caption = "Características basales de las variables de estudio entre diabéticos con y sin TBC [Medias,Sd]")

descrTable(formu,data=dades %>% filter(grup=="Diabetis"),show.p.overall = T,show.n = T,method = 2,Q1=0,Q3=1) %>%
  export2md(caption = "Características basales de las variables de estudio entre diabéticos con y sin TBC [Medianas,Min,Max]")
...
formu<-formula_compare(x="basalesJosep2",y="HbA1c_7",taulavariables = conductor_variables)
...
res5<-dades %>% split(.$HbA1c_7) %>% map_dfr(~Resum_taxa_incidencia(.x,evento="event_tbc",temps="anys_lliure_tbc",valorevent="1"),.id = "Group")  %>% filter(Group=="Si")

res6<-dades %>% split(.$HbA1c_7) %>% map_dfr(~Resum_taxa_incidencia(.x,evento="event_tbc",temps="anys_lliure_tbc",valorevent="1"),.id = "Group")  %>% filter(Group=="No")
...
res1 %>% bind_rows(res5)  %>% bind_rows(res6)  %>%
  kable(digits = 2,caption = "Table 3b. Incidence of TBc with DM patients  for  HbA1c<7.5% and HbA1c>=7.5%  ") %>% kableExtra::kable_styling()
...
#10.3.2022:
descrTable(surv_tbc~HbA1c_7,data=dades,show.ratio = T, byrow = T ,surv = T) %>% export2md(caption = "Incidència i Hazard Ratio de Tuberculosis")
...
```{r genero_HR, message=FALSE, warning=FALSE, include=T, echo=FALSE,size="huge",eval = T}
...
```{r figura_CP_RISK, include=T, fig.asp=1, results="asis",eval = T}
...
```{r resultats8, message=FALSE, warning=FALSE, include=T, echo=FALSE,size="huge",eval = T}
...
```{r testimputacions, eval=T, include=T}
...
```{r,message=FALSE, warning=FALSE, include=T, echo=FALSE,size="huge",eval = T}
...
```{r figura_COX, include=T, fig.asp=1, results="asis", eval=T, include=T}
...
```{r filtre_agencia,include=T,eval = T}
...
```{r resultats_agencia,include=T,eval = T}
dt_temp<-dt_agencia
