...
dades<-dades%>%mutate(HbA1c_10=ifelse(HBA1c.valor>=10,"Si","No"))
...
```{r JOPSEP: Caractèristiques TBC segons grau de control DM,include=T}

#[07.03.2022]#

#table(dades_DM$VACUNA)
...
## TABLA 2. Características basales de los pacientes en cohorte diabética
...
descrTable(formu,data=dadesDM,show.p.overall = T,show.n = T,method = 1,Q1=0,Q3=1,show.all = TRUE) %>%
  export2md(caption = "Tabla  1. Características basales de las variables de estudio entre diabéticos con y sin TBC [Medias,Sd]")

descrTable(formu,data=dadesDM,show.p.overall = T,show.n = T,method = 2,Q1=0,Q3=1,show.all = TRUE) %>%
  export2md(caption = "Tabla  1. Características basales de las variables de estudio entre diabéticos con y sin TBC [Medianas,Min,Max]")



#table(dadesDM$EV.TBC_cat,dadesDM$DG.TBC_cat)
#KKK<-dadesDM%>%select(case.id,DG.TBC_cat, EV.TBC_cat)
      ...
      ggplot(data = dt, aes(HBA1c.valor))+
       geom_histogram(bins = bins,aes(y =1*( ..count../sum(..count..)))) +
        scale_y_continuous(name = "%", labels=scales::percent) +
          labs(title=paste0("Distribució dels HBA1c "),
...
dades %>% filter(grup=="Diabetis" & EV.TBC_cat=="Si") %>% genera_figures_HTG("Distribució % dels valors HBA1c  per TBC",lim2=20,bins=18)
dades %>% filter(grup=="Diabetis" & EV.TBC_cat=="No") %>% genera_figures_HTG("Distribució % dels valors HBA1c  per No TBC",lim2=20)



#ggplot(data = dadesDM2, aes(HBA1c.valor))+
#   geom_histogram(bins = 10,aes(y =1*( ..count../sum(..count..)))) +
#    scale_y_continuous(name = "%", labels=scales::percent) +
#      labs(title="Distribució % dels valors HBA1c per decils agrupats per TBC", x ="valors HBA1c", y = "Percentatge")+
#        scale_color_manual(values=c("#999999", "#E69F00", "#56B4E9"))+
#         scale_fill_manual(values=c("#999999", "#E69F00", "#56B4E9"))+
#           theme_classic()
...
#ggplot(data = dadesDM3, aes(HBA1c.valor))+
#   geom_histogram(bins = 10,aes(y =1*( ..count../sum(..count..)))) +
#    scale_y_continuous(name = "%", labels=scales::percent) +
#      labs(title="Distribució % dels valors HBA1c per decils agrupats per No TBC", x ="valors HBA1c", y = "Percentatge")+
#        scale_color_manual(values=c("#999999", "#E69F00", "#56B4E9"))+
#         scale_fill_manual(values=c("#999999", "#E69F00", "#56B4E9"))+
#           theme_classic()
...
formu<-formula_compare(x="basalesJosep2",y="HbA1c_7.5",taulavariables = conductor_variables)
...
descrTable(formu,data=dadesDM%>%filter(EV.TBC_cat=="Si"),show.p.overall = T,show.n = T,method = 1,Q1=0,Q3=1,show.all = TRUE) %>%
  export2md(caption = "Tabla  2. Características TBC en Poblacion Diabetica según grado de control de la DM <7.5% us >=7.5% [Medias,Sd]")


descrTable(formu,data=dadesDM%>%filter(EV.TBC_cat=="No"),show.p.overall = T,show.n = T,method = 1,Q1=0,Q3=1,show.all = TRUE) %>%
  export2md(caption = "Tabla  2. Características NO TBC en Poblacion Diabetica  según grado de control de la DM <7.5% us >=7.5% [Medias,Sd]")
...
#dades$DG.AVC_cat [avc]
#dades$DG.CARD_ISQU_cat
#dades$DG.ARTPER_cat
#daddes$DG.DM_OFTALMO_cat


#table(dades$IMC.valor.cat,dades$IMC.valor.cat2)

```

## TABLA 3. Incidencia de TB en pacientes DM y no DM
...
res1b %>% bind_rows(res5)  %>%
  kable(digits = 2,caption = "Tabla 3b. Incidencia de TBc con pacientes DM para HbA1c>=7,5%  ") %>% kableExtra::kable_styling()
...
#10.3.2022:
descrTable(surv_tbc~HbA1c_7.5,data=dadesDM,show.ratio = T, byrow = T ,surv = T) %>% export2md(caption = "Incidència i Hazard Ratio de Tuberculosis pacients amb DM")
...
```{r genero_HR, eval=F, echo=FALSE, message=FALSE, warning=FALSE, include=T, size="huge"}
...
```{r testimputacions, eval=T, include=F}
...
```{r figura3, include=T,eval = FALSE}
## Figura d'incidencia acumulada de TB per grup  control DM
# segons K-Meier (TBC)
...
## Tabla.3 Modelos. Análisis multivariante de los factores que pueden influir en la aparición de una nueva TBC, en una Población Diabetica
...
res.cox %>% sjPlot::tab_model(show.p = TRUE,title = "Model 1")
...
res.cox %>% sjPlot::tab_model(show.p = TRUE,title = "Model 2")
...
res.cox %>% sjPlot::tab_model(show.p = TRUE,title = "Model 3")
...
res.cox %>% sjPlot::tab_model(show.p = TRUE,title = "Model 4")
...
res.cox %>% sjPlot::tab_model(show.p = TRUE,title = "Model 5")
...
res.cox %>% sjPlot::tab_model(show.p = TRUE,title = "Model 6")

#[surv_tbc ~ sexe+age+DM_anys+MACRO+MICRO+DG.TAB_cat+DG.ALCOHOL_cat+IMC.valor+HbA1c_7.5]#vii) N=1726
...
res.cox %>% sjPlot::tab_model(show.p = TRUE,title = "Model 7")

#[surv_tbc ~ sexe+age+DM_anys+MACRO+MICRO+DG.TAB_cat+DG.ALCOHOL_cat+HbA1c_7.5]#viii) N=3323
...
res.cox %>% sjPlot::tab_model(show.p = TRUE,title = "Model 8")


#[surv_tbc ~ sexe+age+DM_anys+MACRO+MICRO+DG.TAB_cat+DG.ALCOHOL_cat+HbA1c_7.5]#ix) N=3323
...
res.cox %>% sjPlot::tab_model(show.p = TRUE,title = "Model 9")
...
res.cox %>% sjPlot::tab_model(show.p = TRUE,title = "Model Final1:Analisi Multivariant dels factors que en el DM poden influir al TBC")

# MODEL2::N=1726
if (params$metode=="dinamica2") res.cox<-coxph(surv_tbc ~ sexe+age+DM_anys+MACRO+DG.ALCOHOL_cat+IMC.valor+HbA1c_7.5, data=dadesDM) 
res.cox %>% sjPlot::tab_model(show.p = TRUE,title = "Model Final2:Analisi Multivariant dels factors que en el DM poden influir al TBC")



# MODEL3::n=3323
if (params$metode=="dinamica2") res.cox<-coxph(surv_tbc ~ sexe+age+DM_anys+MACRO+HbA1c_7.5, data=dadesDM) 
res.cox %>% sjPlot::tab_model(show.p = TRUE,title = "Model Final3:Analisi Multivariant dels factors que en el DM poden influir al TBC")


# MODEL4::n=3323
#if (params$metode=="dinamica2") res.cox<-coxph(surv_tbc ~ sexe+age+DM_anys+MACRO+DG.TAB_cat+DG.ALCOHOL_cat+HBA1c.valor, data=dadesDM) 
#res.cox %>% sjPlot::tab_model(show.p = TRUE,title = "Model Final4:Analisi Multivariant dels factors que en el DM poden influir al TBC")
...
dadesDM %>% filter(event_tbc==1) %>% select(CIP,TB__PULMONAR,TB_PLEURAL)

names(dadesDM)

```
## Tabla.4a Modelos de Cox dependientes del Tiempo (HBA1c) para la TBC.[Hba1c>7.5+Sexe+Edat+cluster(idp]
...
## Tabla.4b Modelos de Cox dependientes del Tiempo (HBA1c) para la TBC.[Hba1c>8+Sexe+Edat+cluster(idp]

```{r MODEL_COX2, include=T,eval = TRUE}

#ii)
fit3 <- coxph(Surv(tstart, tstop, situacio=="TBC") ~ val_8+sexe+edat+cluster(idp),data=analitiques_temps)
summary(fit3)
cox.zph(fit3)
...
## Tabla.4c Modelos de Cox dependientes del Tiempo (HBA1c) para la TBC.[Hba1c>9+Sexe+Edat+cluster(idp]

```{r MODEL_COX3, include=T,eval = TRUE}

#iii)
fit2 <- coxph(Surv(tstart, tstop, situacio=="TBC") ~ val_9+sexe+edat+cluster(idp),data=analitiques_temps)
summary(fit2)
cox.zph(fit1)
