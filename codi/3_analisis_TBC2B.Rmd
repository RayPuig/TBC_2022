...
kkk<-kkk%>% filter(CIP!="GARR144010100" |  CIP=="GARR144010100" & TB_PLEURAL=="2")
...
res5<-dadesDM %>% split(.$HbA1c_7) %>% map_dfr(~Resum_taxa_incidencia(.x,evento="event_tbc",temps="anys_lliure_tbc",valorevent="1"),.id = "Group") 
...
#10.3.2022:
descrTable(surv_tbc~HbA1c_7,data=dadesDM,show.ratio = T, byrow = T ,surv = T) %>% export2md(caption = "Incidència i Hazard Ratio de Tuberculosis pacients amb DM")
...
if (params$metode=="dinamica2") res.cox<-coxph(surv_tbc ~ HbA1c_7, data=dadesDM) 
res.cox %>% sjPlot::tab_model(show.p = TRUE)

if (params$metode=="dinamica2") res.cox<-coxph(surv_tbc ~ HbA1c_7+sexe, data=dadesDM) 
res.cox %>% sjPlot::tab_model(show.p = TRUE)


if (params$metode=="dinamica2") res.cox<-coxph(surv_tbc ~ HbA1c_7+sexe+age, data=dadesDM) 
res.cox %>% sjPlot::tab_model(show.p = TRUE)
