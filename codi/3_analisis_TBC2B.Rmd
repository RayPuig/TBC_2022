...
descrTable(formu,data=dadesDM,show.p.overall = T,show.n = T,method = 1,Q1=0,Q3=1,show.all = TRUE) %>%
  export2md(caption = "Tabla  2b. Características basales de las variables de estudio entre diabéticos con y sin TBC [Medias,Sd]")

descrTable(formu,data=dadesDM,show.p.overall = T,show.n = T,method = 2,Q1=0,Q3=1,show.all = TRUE) %>%
  export2md(caption = "Tabla  2b. Características basales de las variables de estudio entre diabéticos con y sin TBC [Medianas,Min,Max]")
...
descrTable(formu,data=dadesDM%>%filter(EV.TBC_cat=="Si"),show.p.overall = T,show.n = T,method = 1,Q1=0,Q3=1,show.all = TRUE) %>%
  export2md(caption = "Tabla  2c. Características TBC en Poblacion Diabetica según grado de control de la DM <7.5% us >=7.5% [Medias,Sd]")


descrTable(formu,data=dadesDM%>%filter(EV.TBC_cat=="No"),show.p.overall = T,show.n = T,method = 1,Q1=0,Q3=1,show.all = TRUE) %>%
  export2md(caption = "Tabla  2c. Características NO TBC en Poblacion Diabetica  según grado de control de la DM <7.5% us >=7.5% [Medias,Sd]")
...
```
## Dependenet Time Variable DM Cox   HBA1c us TBC [val_7_5+strata(sexe)+edat_CAT+cluster(idp)]
```{r MODEL10, include=T,eval = TRUE}
...
analitiques_temps<-analitiques_temps%>%mutate(val_7_5 = ifelse(val<7.5 | is.na(val)   , 0, 1))
analitiques_temps<-analitiques_temps%>%mutate(val_8 =   ifelse(val<8   | is.na(val)   , 0, 1))
analitiques_temps<-analitiques_temps%>%mutate(val_9 =   ifelse(val<9   | is.na(val)   , 0, 1))
